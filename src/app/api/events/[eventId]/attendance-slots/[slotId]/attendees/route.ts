...
export async function GET(
  ...
  try {
    ...
    let query = supabase
      ...
      .select(
        ...
        students!inner(
          *,
          degrees!left(id, name),
          programs!left(id, name),
          majors!left(id, name),
          year_levels!left(id, name)
        )
      `
      )
  ...
  `,
    ...
    // Make sure to use the same programId variable from above
    if (programId !== undefined) {
      countQuery = countQuery.eq("students.program_id", programId);
    }

    if (yearLevelId !== undefined) {
      countQuery = countQuery.eq("students.year_level_id", yearLevelId);
    }
    // Only get the count
