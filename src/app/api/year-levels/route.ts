...
import { GetYearLevelsRequest } from "@/lib/requests/year-levels/get";
import { NextRequest, NextResponse } from "next/server";

// GET /api/year-levels - List year-levels
export async function GET(request: NextRequest) {
  const customRequest = new GetYearLevelsRequest(request);
  ...
  try {
    ...
    let query = supabase.from("year_levels").select(
      `
        *
      `,
      { count: "exact" }
    ...
    return NextResponse.json(
      {
        year_levels:
          data?.map(year_level =>
            Object.fromEntries(
              Object.entries(year_level).sort(([keyA], [keyB]) =>
