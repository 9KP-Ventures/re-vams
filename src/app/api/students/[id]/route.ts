...
export async function GET(
  ...
  const customRequest = new GetStudentRequest(request, id);
...
export async function PATCH(
  ...
  const { id } = await params;
  const customRequest = new UpdateStudentRequest(request, id);
  const validationError = await customRequest.validate();
  ...
  try {
    ...
    if (updateData.program_id || updateData.degree_id || updateData.major_id) {
      const checks = [];
          ...
          return NextResponse.json(
            {
              error: {
                code: 400,
                message: `Major with ID ${updateData.major_id} does not exist.`,
              },
            },
            { status: 400 }
          );
