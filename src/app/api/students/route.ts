...
import { CreateStudentRequest } from "@/lib/requests/students/create";
import { GetStudentsRequest } from "@/lib/requests/students/get-many";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Build the query with corrected relationships
    ...
    if (customRequest.getSearch()) {
      ...
      query = query.or(
        `id.ilike.%${search}%,first_name.ilike.%${search}%,last_name.ilike.%${search}%,email_address.ilike.%${search}%`
      );
    }
